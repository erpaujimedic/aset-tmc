...
@router.get("")
def get_tickets(branch: Optional[str] = None, limit: Optional[int] = None):
    if not supabase:
        raise HTTPException(status_code=500, detail="Database connection error")
    try:
        all_data = []
...
async def upload_file(
    ...
    origin_branch: Optional[str] = Form(None),
    dest_branch: Optional[str] = Form(None)
):
    try:
        ...
        if module == "kalibrasi":
            folder_id = get_or_create_folder(service, "Kalibrasi")
            file_name_base = generate_filename("naming_format_calibration", asset_code, file.filename, "System", asset_name)
            file_name = f"{file_name_base}.{file_ext}" if not file_name_base.endswith(f".{file_ext}") else file_name_base
        elif module == "logistik":
            folder_id = get_or_create_folder(service, "Logistics & Tracking")
            file_name_base = generate_filename("naming_format_dispatch", asset_code, file.filename, "System", asset_name)
            file_name = f"{file_name_base}.{file_ext}" if not file_name_base.endswith(f".{file_ext}") else file_name_base
        elif module == "asset":
            folder_id = get_or_create_folder(service, "Asset Photos")
            file_name_base = generate_filename("naming_format_asset_photo", asset_code, file.filename, "System", asset_name)
            file_name = f"{file_name_base}.{file_ext}" if not file_name_base.endswith(f".{file_ext}") else file_name_base
        else:
            folder_id = get_or_create_folder(service, "Tickets")
            file_name = f"ticket_photo_{uuid.uuid4().hex[:8]}_{today_str}.{file_ext}"
            
        # upload to drive
        file_url = upload_file_to_drive(file.file, file_name, file.content_type, folder_id)
        return {"url": file_url}
    except Exception as e:
...
async def upload_signed_form(
    ...
    try:
        ...
        # Log to history first so we can update it if needed, or just update the ticket
        # Update database with placeholder
        res = supabase.table("tickets").update({"signed_form_url": url}).eq("id", ticket_id).execute()
        
        updates_for_bg = [
            {"table": "tickets", "id": ticket_id, "column": "signed_form_url"}
        ]
        ...
        return {"message": "Form uploaded successfully and is processing"}
    except Exception as e:
