...
export default function Roles() {
  ...
  const canCreate = hasPermission('User Managements - Roles', 'Create');
  const canEdit = hasPermission('User Managements - Roles', 'Edit');
  const canDelete = hasPermission('User Managements - Roles', 'Delete');
          ...
          <div className="p-4 border-b border-slate-100 bg-white z-10 relative">
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="flex gap-2">
                <button 
                  onClick={async () => {
                    try {
                      ...
                      if (selectedRole) {
                        setLoading(true);
                        const perms = await fetchRolePermissions(selectedRole);
                ...
                >
                  <svg className={`w-3.5 h-3.5 ${loading ? 'animate-spin' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5"><path strokeLinecap="round" strokeLinejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
                </button>
                {canCreate && (
          ...
          <div className="p-5 border-b border-slate-100 flex items-center justify-between bg-white z-10 shadow-sm">
            <div>
              ...
              <h2 className="text-xl font-black text-[#286086] flex items-center gap-2 mt-1">
                ...
                {selectedRole.toLowerCase() !== 'master admin' && canDelete && (
            ...
            <div className="flex items-center gap-3">
              ...
              <label className={`relative inline-flex items-center ${canEdit ? 'cursor-pointer' : 'cursor-not-allowed opacity-60'}`}>
                <input type="checkbox" className="sr-only peer" checked={selectAll} onChange={handleSelectAll} disabled={!canEdit} />
              ...
              {permissions.map((group, idx) => (
                <div key={idx} className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                  <div className="px-5 py-3 border-b border-slate-100 bg-slate-50/80">
                    <h3 className="font-bold text-slate-700 text-sm">{t(group.module) || group.module}</h3>
                  </div>
                  <div className="p-1.5">
                    {group.actions.map((action, aIdx) => (
                      <div key={aIdx} className="flex items-center justify-between px-3 py-1.5 rounded-lg hover:bg-slate-50 transition-colors">
                        <span className="text-sm font-medium text-slate-600">{t(action.name) || action.name}</span>
                        <label className={`relative inline-flex items-center ${canEdit ? 'cursor-pointer' : 'cursor-not-allowed opacity-60'}`}>
                          <input type="checkbox" className="sr-only peer" checked={action.enabled} onChange={() => togglePermission(idx, aIdx)} disabled={!canEdit} />
